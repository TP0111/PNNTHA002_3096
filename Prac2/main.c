...
uint16_t size = sizeof(patterns); //size of the patterns
uint8_t data ; // data written in an address
static uint16_t currentIndex = 0; // variable to keep track of index in the array
static uint16_t memAddress = 0 ; // variable to keep track of the binary array index
static uint8_t delayState = 0; // variable to track the delay state
/* USER CODE END PV */
...
int main(void)
{
  ...
  for(uint16_t i = 0; i < size; i++) {
    write_to_address(memAddress, patterns[i]);
    memAddress ++;
  }
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // TODO: Check button PA0; if pressed, change timer delay
	    if (!HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin)) {
	      if (htim16.Init.Period == 1000 - 1 && delayState == 0) {
	        htim16.Init.Period = 500;
	      } else if (htim16.Init.Period == 500 && delayState == 0) {
	        htim16.Init.Period = 1000 - 1;
	     }
	      // Reinitialize and restart TIM16
...
void TIM16_IRQHandler(void)
{
	...
	// TODO: Change to next LED pattern; output 0x01 if the read SPI data is incorrect
	    ...
	    data = read_from_address(currentIndex);
		// Check if the read data matches the pattern at the current index
		if (data == patterns[currentIndex]) {
		    GPIOB->ODR = patterns[currentIndex];
		} else {
		    GPIOB->ODR = 0b00000001;
		}
		currentIndex++;

		// Check if the index exceeds the pattern size
