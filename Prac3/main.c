...
void EXTI0_1_IRQHandler(void)
{
	// TODO: Add code to switch LED7 delay frequency
	  ...
	  // Check if the interrupt is generated by EXTI0 (PA0)
	  if (__HAL_GPIO_EXTI_GET_IT(GPIO_PIN_0) != RESET)
	  {
	    // Clear the interrupt flag
	    __HAL_GPIO_EXTI_CLEAR_IT(GPIO_PIN_0);

	    // Check the time since the last frequency change
	    curr_millis = HAL_GetTick();
	    if (curr_millis - prev_millis >= 200) // Minimum delay between frequency changes is 200ms
	    {
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	// Calculate the CCR value based on the ADC value
	uint32_t crr_val = (adc_val * 47999) / 4095;
